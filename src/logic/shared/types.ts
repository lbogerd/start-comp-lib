...
export const FileSchema = FileBaseSchema.superRefine((file, ctx) => {
  if (PageOrFile.safeParse(file.type).success && !file.target) {
    ctx.addIssue({
      code: "custom",
      message: "`target` is required when file.type is 'registry:page' or 'registry:file'",
      path: ["target"],
    });
  }
});
...
const ComponentSchema  = BaseItemSchema.extend({ type: z.literal("registry:component") });
...
const FileItemSchema   = BaseItemSchema.extend({ type: z.literal("registry:file") });
...
export const RegistrySchema = z.object({
    name: z.string().meta({
        title: "Registry Name",
        description: "The name of the registry. This is used for data attributes and other metadata.",
        example: "acme"
    }),
    homepage: z.url().meta({
        title: "Homepage",
        description: "The homepage of the registry. This is used for data attributes and other metadata.",
        example: "http://localhost:3000/libs/acme"
    }),
    items: z.array(RegistryItemSchema).meta({
        title: "Items",
        description: "The items in the registry. Each item must implement the registry-item schema specification.",
    })
})
